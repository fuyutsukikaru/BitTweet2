...
class FavsFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater?, container: ViewGroup?, savedInstanceState: Bundle?) : View {
        ...
        val adapter = TweetsAdapter(activity, userId, this@FavsFragment)
        ...
        if (adapter.itemCount == 0) {
            val tweets = tinydb.getListObject(username + "favs_timeline", Tweet::class.java) as List<Tweet>
    ...
    fun refresh(refreshLayout: SwipeRefreshLayout, adapter: TweetsAdapter, tinydb: TinyDB, username: String) {
        ...
        statusesService.userTimeline(null, "pan_ningen", 200, lastId, null, false, false, true, true, object: Callback<List<Tweet>>() {
            override fun success(result: Result<List<Tweet>>) {
                adapter.putTweets(result.data)

                // Store new timeline objects in persistent storage and update
                val temp = tinydb.getListObject(username + "favs_timeline", Tweet::class.java)
                temp.addAll(0, result.data)
                tinydb.putListObject(username + "favs_timeline", temp)
